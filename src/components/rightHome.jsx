...
const RightHome = () => {
    ...
    return (
        <div className="w-[20%] h-[100%] p-2 items-center flex flex-col space-y-2">
            <input className="rounded-[100px] px-4 py-2 w-full bg-gray-700 focus:outline-none" type="text" name='search' id='search' placeholder='Search' onChange={handleSearchChange} onKeyDown={handleSearch} value={searchName} />
            <div className="w-full rounded-xl bg-gray-800 flex flex-col justify-center overflow-auto">
                {searchResults && searchResults.map((user, index) => (
                    <div key={index} onClick={()=>{handleClick(user)}} className="flex gap-2 p-4 border-b-solid border-b-gray-900 border-b-[2px] cursor-pointer">
                        {user.name ?(
                        <>
